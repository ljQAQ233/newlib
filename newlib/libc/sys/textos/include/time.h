/**
 * @file
 * @brief textos <time.h> override: expose nanosleep.
 *
 * textos implements the nanosleep syscall, but newlib only declares it when
 * the target advertises POSIX timers (see <sys/features.h>).  textos' own
 * <time.h> declares it unconditionally, so do the same here.
 *
 * @author maouai233
 */
...
clock(void);
double
difftime(time_t _time2, time_t _time1);
time_t
...
#ifndef _REENT_ONLY
char*
asctime(const struct tm* _tblock);
char*
ctime(const time_t* _time);
struct tm*
...
char*
asctime_r(const struct tm* __restrict, char* __restrict);
char*
ctime_r(const time_t*, char*);
struct tm*
gmtime_r(const time_t* __restrict, struct tm* __restrict);
struct tm*
localtime_r(const time_t* __restrict, struct tm* __restrict);
...
#endif
#if __GNU_VISIBLE
  char* strptime_l(const char* __restrict,
                   const char* __restrict,
                   struct tm* __restrict,
                   locale_t);
#endif
